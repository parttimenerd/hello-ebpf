...
/**
 * A BPF type, see <a href="https://www.kernel.org/doc/html/latest/bpf/btf.html">Linux BTF
...
public sealed interface BPFType {
    ...
    record BPFIntType(
        ...
        public static BPFIntType UINT64 =
                new BPFIntType(
                        "u64",
                        ...
                        new AnnotatedClass(long.class, List.of(AnnotationInstances.UNSIGNED)),
                        0);

        /** <code>uint32_t</code> mapped to {@code @Unsigned int} */
        public static BPFIntType UINT32 =
                new BPFIntType(
                        "u32",
                        ...
                        new AnnotatedClass(int.class, List.of(AnnotationInstances.UNSIGNED)),
                        0);

        /** <code>int32_t</code> mapped to {@code int} */
        public static BPFIntType INT32 =
                new BPFIntType(
                        "s32",
                        ValueLayout.JAVA_INT,
                        segment -> {
                        ...
                        new AnnotatedClass(int.class, List.of()),
                        ENCODING_SIGNED);

        /** <code>char</code> mapped to {@code char} */
        public static BPFIntType CHAR =
                new BPFIntType(
                        "char",
                        ...
                        new AnnotatedClass(byte.class, List.of()),
                        ENCODING_CHAR);
    }
    ...
    record BPFStructType(
        ...
        public long size() {
            return members.stream()
                    .mapToLong(member -> member.type.size() + member.offset)
                    .max()
                    .orElseThrow();
        }
            ...
            return segment -> {
                List<Object> args =
                        members.stream()
                                .map(
                                        member ->
                                                member.type.parseMemory(
                                                        segment.asSlice(member.offset)))
                                .toList();
        ...
        public MemorySetter setter() {
            return (segment, obj) -> {
                for (BPFStructMember member : members) {
                    member.type.setMemory(
                            segment.asSlice(member.offset),
                            ((Function<Object, Object>) member.getter).apply(obj));
                }
            };
        }
    }
        ...
        public MemoryParser parser() {
            return segment ->
                    IntStream.range(0, length)
                            .mapToObj(
                                    i ->
                                            memberType.parseMemory(
                                                    segment.asSlice(i * memberType.size())))
                            .toList();
        }
