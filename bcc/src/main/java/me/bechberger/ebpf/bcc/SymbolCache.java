 ...
 *
 * <p>Copied from the bcc Python bindings
 */
public class SymbolCache {
    ...
    /**
     * Return a tuple of the symbol (function), its offset from the beginning of the function, and
     * the module in which it lies. For example: ("start_thread", 0x202,
     * "/usr/lib/.../libpthread-2.24.so") If the symbol cannot be found but we know which module it
     * is in, return the module name and the offset from the beginning of the module. If we don't
     * even know the module, return the absolute address as the offset.
     */
            ...
            if (res < 0) {
                if (bcc_symbol.module$get(sym) != null && bcc_symbol.offset$get(sym) != 0) {
                    return new ResolveResult(
                            null,
                            bcc_symbol.offset$get(sym),
                            PanamaUtil.toString(bcc_symbol.module$get(sym)));
                }
