...
/**
 * Parse helpers for IP packets, available to any BPF program.
 *
 * <p>Implement this interface in a {@code @BPF} class alongside {@link XDPHook} or {@link TCHook}
 * to get Ethernet/IP/TCP/UDP parsing. Bridge methods
 * {@link #parsePacket(XDPContext, Ptr)} and {@link #parsePacket(TCContext, Ptr)} extract
 * {@code data}/{@code data_end} from the respective context types automatically.
 *
 * <p>Based on the examples from the
 * <a href="https://github.com/xdp-project/bpf-examples">xdp-project</a>.
 */
@BPFInterface
public interface BasePacketParser {
    ...
    /** Parse the transport-layer header and fill port/protocol fields in {@code info}. */
    @BPFFunction
    @AlwaysInline
    ...
    /** Parse an IPv4 packet, filling address and port fields in {@code info}. */
    @BPFFunction
    @AlwaysInline
    ...
    /** Parse an IPv6 packet (including extension headers), filling {@code info}. */
    @BPFFunction
    @AlwaysInline
    ...
    /** Parse a packet from raw data/end pointers (uint32 form, as in xdp_md). */
    @BPFFunction
    @AlwaysInline
    ...
    /** Parse a packet from void* start/end pointers. */
    @BPFFunction
    @AlwaysInline
    ...
    /**
     * Parse a full packet from an XDP context.
     *
     * <p>Convenience bridge: extracts {@code data}/{@code data_end} from {@code ctx} and
     * delegates to {@link #parsePacket(Ptr, Ptr, Ptr)}.
     */
    @BPFFunction(name = "parsePacketXDP", callTemplate = "parsePacketXDP($arg1, $arg2)")
    @AlwaysInline
    default boolean parsePacket(XDPContext ctx, Ptr<PacketInfo> info) {
        return parsePacket2(ctx.data(), ctx.dataEnd(), info);
    }

    /**
     * Parse a full packet from a TC context.
     *
     * <p>Convenience bridge: extracts {@code data}/{@code data_end} from {@code skb} and
     * delegates to {@link #parsePacket(Ptr, Ptr, Ptr)}.
     */
    @BPFFunction(name = "parsePacketTC", callTemplate = "parsePacketTC($arg1, $arg2)")
    @AlwaysInline
    default boolean parsePacket(TCContext skb, Ptr<PacketInfo> info) {
        return parsePacket2(skb.data(), skb.dataEnd(), info);
    }
}
