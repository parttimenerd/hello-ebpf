...
**Tech Stack:** Java 25, Panama FFI (`me.bechberger.ebpf.bpf.raw.Lib` — generated by jextract from `<bpf/bpf.h>` and `<bpf/btf.h>`), JUnit 5, virtme-ng for real-kernel tests. Kernel floor is 6.14+.
...
- Read: `rawbpf/misc/bpf_headers.h` — confirm which headers jextract sees.
- Read: `rawbpf/bin/jextract_bindings.py` — confirm output structure.
- Read: `bpf/src/main/java/me/bechberger/ebpf/bpf/BPFProgram.java:1-100` — confirm `Lib.*` import path.

- [ ] **Step 1: Confirm generated FFI names**

Rebuild `rawbpf` on thinkstation so the generated `Lib.java` exists, then grep for the symbols this plan needs:

```
ssh thinkstation 'cd /home/i560383/code/experiments/hello-ebpf && HOME=/home/i560383 JAVA_HOME=/home/i560383/.sdkman/candidates/java/25-sapmchn mvn -pl rawbpf -am compile'
ssh thinkstation 'cd /home/i560383/code/experiments/hello-ebpf && grep -E "public static (int|long|MemorySegment) (bpf_prog_load|bpf_map_create|bpf_link_create|btf__parse|btf__parse_raw|btf__name_by_offset|btf__type_by_id|btf__type_cnt|btf__find_by_name|btf__find_by_name_kind|libbpf_get_error)\\b" rawbpf/src/main/java/me/bechberger/ebpf/bpf/raw/Lib.java | head -40'
```

Expected: every symbol listed above resolves to a `public static` method on `Lib`. Record which parameter overloads exist (some jextract-generated methods have `_opts` variants). If `btf__parse_raw` is absent, use `btf__parse` on `/sys/kernel/btf/vmlinux` instead.

- [ ] **Step 2: Confirm `/sys/kernel/btf/vmlinux` is readable and no code currently reads it**
...
Expected: file exists and is world-readable (mode 444). No Java references. Result: BTF loading path is greenfield.

- [ ] **Step 3: Confirm `KernelFeatures.currentKernelVersion` exists in `shared`**
...
- [ ] **Step 4: Commit an inventory note (empty commit)**
...
public final class ProgramTypeProbe {
    ...
    public static ProbeResult probe(BPFProgramType type) {
        try (Arena arena = Arena.ofConfined()) {
            ...
            ProbeSyscallCounter.increment();
            int fd = Lib.bpf_prog_load(
                    type.id(),
                    MemorySegment.NULL,   // prog_name (NULL is fine for probes)
                    license,
                    insns,
                    INSNS_RETURN0.length / 8L,  // insn count
                    MemorySegment.NULL);          // opts

            if (fd >= 0) {
                // Close the fd immediately; we only wanted to know if the kernel accepts the type.
                try { me.bechberger.ebpf.shared.LibC.close(fd); } catch (Throwable ignore) {}
                return new ProbeResult.Supported();
            }

            int err = -fd;  // libbpf returns -errno on failure
            return switch (err) {
...
Note the `LibC.close` — hello-ebpf already exposes `me.bechberger.ebpf.shared.LibC` (see BPFProgram imports). If `close` is not on `LibC`, use `Lib.close(fd)` (jextract exposes it via `<unistd.h>`).
...
public final class MapTypeProbe {
    ...
    public static ProbeResult probe(MapTypeId t) {
        ...
        try {
            ProbeSyscallCounter.increment();
            int fd = Lib.bpf_map_create(
                    t.getId(),
                    MemorySegment.NULL,  // name (NULL ok for probes)
                    s.keySize(),
                    s.valueSize(),
                    s.maxEntries(),
                    MemorySegment.NULL); // opts
...
public final class BtfLoader {
    ...
    private static volatile Arena BTF_ARENA = null;
    ...
    private static void ensureLoaded() {
        ...
        synchronized (LOCK) {
            ...
            try {
                ...
                MemorySegment btf = Lib.btf__parse(path, MemorySegment.NULL);
                long err = Lib.libbpf_get_error(btf);
                if (err != 0 || btf == null || btf.address() == 0) {
                    arena.close();
                    BTF_ARENA = null;
                    BTF = null;
                    AVAILABLE = false;
    ...
    private static boolean findByName(String name, int kind) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            int id = Lib.btf__find_by_name_kind(BTF, n, kind);
            return id > 0;
        } catch (Throwable t) {
            return false;
        }
    ...
    static void resetForTest() {
        synchronized (LOCK) {
            if (BTF_ARENA != null) {
                try { BTF_ARENA.close(); } catch (Throwable ignore) {}
            }
            BTF_ARENA = null;
            BTF = null;
            AVAILABLE = null;
...
public final class AttachTypeProbe {
    ...
    public static ProbeResult probe(BPFAttachType t) {
        try {
            ProbeSyscallCounter.increment();
            int fd = Lib.bpf_link_create(
                    -1,             // prog_fd — invalid, but that is fine for the probe
                    -1,             // target_fd — invalid; expect EBADF if attach type known
                    t.id(),
                    MemorySegment.NULL);   // opts
            if (fd >= 0) {
                try { me.bechberger.ebpf.shared.LibC.close(fd); } catch (Throwable ignore) {}
