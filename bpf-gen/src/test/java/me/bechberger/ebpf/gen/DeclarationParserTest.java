...
public class DeclarationParserTest {
    ...
    @ParameterizedTest
    @CsvSource(value = {
            "static void (* const print)(void) = (void*) 0;|void print()",
            "static void (* const print)(int i) = (void*) 0;|void print(int i)",
            "static void (* const print)(int i, int j) = (void*) 0;|void print(int i, int j)",
            ...
            "static int* (* const print)(int i) = (void*) 0;|Ptr<java.lang.Integer> print(int i)",
            "static const int* (* const print)(int i) = (void*) 0;|Ptr<java.lang.Integer> print(int i)",
            // and var args
            ...
            "static long (* const fn)(void (*func)(int a, int b), int a)|long fn(Ptr<?> func, int a)",
            "static long (* const fn)(void (*func)(int a, int (*b)()), int a)|long fn(Ptr<?> func, int a)"
    ...
    @CsvSource(value = {
           /* "void* print(int i);|Ptr<?> print(int i)",
            ...
            "void* vfork(unsigned long addr, unsigned long length, unsigned long prot, unsigned long flags);|Ptr<?> vfork(@Unsigned long addr, @Unsigned long length, @Unsigned long prot, @Unsigned long flags)",
            "pid_t vfork(void);|pid_t vfork()",
            "long vfork3(struct clone_args *cl_args, size_t size);|long vfork3(Ptr<clone_args> cl_args, @Unsigned long size)",
            "int getgroups(int size, gid_t list[]);|int getgroups(int size, Ptr<gid_t> list)",
            "ssize_t write(int fd, const void buf[.count], size_t count);|ssize_t write(int fd, Ptr<?> buf, @Unsigned long count)",
    ...
    void assertParse(String javaDeclaration, FuncType actual) {
        assertEquals(javaDeclaration, actual.toMethodSpec(gen).toString().replace('\n', ' ').replaceAll(" +", " ").split("public static ")[1].split(" \\{")[0]);
    }
